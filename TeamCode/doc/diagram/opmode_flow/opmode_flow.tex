...
\begin{tikzpicture}[node distance=1cm]
	...
	\draw [arrow] (start) -> (create_init_hardware);
	\draw [arrow] (create_init_hardware) -> (populate_state_map);
	\draw [arrow] (populate_state_map) -> (wait_for_start);
	\draw [arrow] (wait_for_start) -> (while_opmode_running);
	...
	\draw [arrow] (determine_next_state_prev) -> (current_on_switch_to);
	\draw [arrow] (current_on_switch_to) -> (next_on_switch_from);
	\draw [arrow] (next_on_switch_from) -> (set_next_active);
	\draw [arrow] (set_next_active) -> (current_state_loop);
